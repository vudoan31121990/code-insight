...
describe('languageService', () => {
	...
	beforeEach(() => {
		...
		global.fetch = jest.fn(() =>
			Promise.resolve({
				ok: true,
				json: () => Promise.resolve({ data: dataResponse.Languages })
			})
		);
	});
	...
	it('should dispatch mock data actions when mockData is enabled', async () => {
		...
		expect(dispatch).toHaveBeenCalledWith({ type: GET_LANGUAGE_LOADING });
		...
		expect(dispatch).toHaveBeenCalledWith({
			...
			payload: dataResponse.Languages
		});
	});
	...
	it('should dispatch success actions when fetch is successful', async () => {
		...
		expect(global.fetch).toHaveBeenCalledWith(
			'https://code-insight-api-dot-code-insight-dev.uc.r.appspot.com/api/languages'
		);
	});
