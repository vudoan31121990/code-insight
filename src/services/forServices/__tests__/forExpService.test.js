import forExpService from '../forExpService';
import {
	GET_FOR_EXP_LOADING,
	GET_FOR_EXP_SUCCESS,
	GET_FOR_EXP_FAILURE
...
describe('forExpService', () => {
	...
	beforeEach(() => {
		...
		global.fetch = jest.fn(() =>
			Promise.resolve({
				ok: true,
				json: () => Promise.resolve({ data: forLoopCodeExpData })
			})
		);
	});
	...
	it('should dispatch mock data actions when mockData is enabled', async () => {
		...
		// Execute the service
		const service = forExpService();
		await service(dispatch);

		expect(dispatch).toHaveBeenCalledWith({ type: GET_FOR_EXP_LOADING });

		// Advance the clock to execute the code inside the setTimeout function
		jest.runOnlyPendingTimers();

		expect(dispatch).toHaveBeenCalledWith({
			type: GET_FOR_EXP_SUCCESS,
			payload: forLoopCodeExpData
		});
	});
	...
	it('should dispatch success actions when fetch is successful', async () => {
		...
		// Assert that `GET_FOR_EXP_LOADING` was dispatched
		expect(dispatch).toHaveBeenCalledWith({ type: GET_FOR_EXP_LOADING });

		// Assert that `GET_FOR_EXP_SUCCESS` was dispatched with the API data
		expect(dispatch).toHaveBeenCalledWith({
			type: GET_FOR_EXP_SUCCESS,
			payload: forLoopCodeExpData
		});
		...
		expect(global.fetch).toHaveBeenCalledWith(
			'https://code-insight-api-dot-code-insight-dev.uc.r.appspot.com/api/for-exp'
		);
	});
