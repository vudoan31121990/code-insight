import ifelseSnippetService from '../ifelseSnippetService';
import {
...
describe('ifelseSnippetService', () => {
	...
	beforeEach(() => {
		...
		global.fetch = jest.fn(() =>
			Promise.resolve({
				ok: true,
				json: () => Promise.resolve({ data: ifelseCodeData })
			})
		);
	});
	...
	it('should dispatch mock data actions when mockData is enabled', async () => {
		...
		// Execute the service
		const service = ifelseSnippetService();
		await service(dispatch);

		expect(dispatch).toHaveBeenCalledWith({ type: GET_IFELSE_SNIPPET_LOADING });
	...
	it('should dispatch success actions when fetch is successful', async () => {
		...
		// Assert that `GET_IFELSE_SNIPPET_LOADING` was dispatched
		expect(dispatch).toHaveBeenCalledWith({ type: GET_IFELSE_SNIPPET_LOADING });

		// Assert that `GET_IFELSE_SNIPPET_SUCCESS` was dispatched with the API data
		expect(dispatch).toHaveBeenCalledWith({
			type: GET_IFELSE_SNIPPET_SUCCESS,
			payload: ifelseCodeData
		});
		...
		expect(global.fetch).toHaveBeenCalledWith(
			'https://code-insight-api-dot-code-insight-dev.uc.r.appspot.com/api/ifelse-code'
		);
	});
	...
	it('should dispatch failure actions when fetch fails', async () => {
		...
		// Assert that `GET_IFELSE_SNIPPET_FAILURE` was dispatched with the error
		expect(dispatch).toHaveBeenCalledWith({
			type: GET_IFELSE_SNIPPET_FAILURE,
