import whileSnippetService from '../whileSnippetService';
import {
...
describe('whileSnippetService', () => {
	...
	beforeEach(() => {
		...
		global.fetch = jest.fn(() =>
			Promise.resolve({
				ok: true,
				json: () => Promise.resolve({ data: whileLoopCodeData })
			})
		);
	});
	...
	it('should dispatch mock data actions when mockData is enabled', async () => {
		...
		// Execute the service
		const service = whileSnippetService();
	...
	it('should dispatch success actions when fetch is successful', async () => {
		...
		expect(global.fetch).toHaveBeenCalledWith(
			'https://code-insight-api-dot-code-insight-dev.uc.r.appspot.com/api/while-code'
		);
	});
	...
	it('should dispatch failure actions when fetch fails', async () => {
		...
		// Assert that `GET_WHILE_SNIPPET_FAILURE` was dispatched with the error
		expect(dispatch).toHaveBeenCalledWith({
			type: GET_WHILE_SNIPPET_FAILURE,
